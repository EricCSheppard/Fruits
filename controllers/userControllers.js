...
router.post('/login', async (req, res) => {
    ...
    User.findOne( { username })
        .then(async (user) => {
            ...
            if (user) {
                //if they do, we compare the passwords
                // bcrypt.compare -> evalues to a truthy or a falsy value
                // we'll save that result to a variable for easy reference later
                // passowrd -> comes from req.body
                //user.password -> saved in the database
...
router.delete('/logout', (req, res) => {
    ...
    req.session.destroy(() => {
        ...
        // eventually we will redirect users here but that's after adding the view layer.
        res.sendStatus(204)
    })
})
